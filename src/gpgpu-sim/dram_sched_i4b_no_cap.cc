...
i4b_no_cap_scheduler::i4b_no_cap_scheduler(const memory_config *config,
...
void i4b_no_cap_scheduler::update_mode() {
    ...
    if (have_pim) {
      ...
      if (pim_threshold_exceeded || !(have_reads || have_writes)) {
        ...
        printf("DRAM (%d): Updating at cycle %lld\n", m_dram->id,
          ...
          if (non_pim_arrival_rate_change > \
              I4B_NO_CAP_NON_PIM_ARR_RATE_TOLERANCE) {
#ifdef DRAM_SCHED_VERIFY
            ...
            m_instability += 2;
            if (m_instability > I4B_NO_CAP_MAX_INSTABILITY) {
                m_instability = 0;
                m_interval_length *= 2;
...
void i4b_no_cap_scheduler::add_req(dram_req_t *req) {
...
dram_req_t *i4b_no_cap_scheduler::schedule(unsigned bank, unsigned curr_row) {
...
dram_req_t *i4b_no_cap_scheduler::schedule_pim() {
...
void i4b_no_cap_scheduler::end_exploratory_phase() {
  ...
  if (!m_stable_state) {
    ...
    if (m_max_pim_batches > I4B_NO_CAP_MAX_PIM_BATCH_SIZE) {
      m_stable_state = true;
