...
i4a_no_cap_scheduler::i4a_no_cap_scheduler(const memory_config *config,
  ...
  m_stable_state = true;
  m_next_update_cycle = I4A_NO_CAP_INTERVAL_LENGTH;
...
void i4a_no_cap_scheduler::update_mode() {
...
void i4a_no_cap_scheduler::add_req(dram_req_t *req) {
...
dram_req_t *i4a_no_cap_scheduler::schedule(unsigned bank, unsigned curr_row) {
...
dram_req_t *i4a_no_cap_scheduler::schedule_pim() {
...
void i4a_no_cap_scheduler::end_exploratory_phase() {
  ...
  if (!m_stable_state) {
    ...
    if (m_max_pim_batches > I4A_NO_CAP_MAX_PIM_BATCH_SIZE) {
      m_stable_state = true;
