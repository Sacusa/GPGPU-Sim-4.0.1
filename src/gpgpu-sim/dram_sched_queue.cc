...
queue_scheduler::queue_scheduler(const memory_config *config, dram_t *dm,
    memory_stats_t *stats) : dram_scheduler(config, dm, stats)
{
  ...
  m_next_update_cycle = DRAM_SCHED_QUEUE_PHASE_LENGTH;
...
void queue_scheduler::update_mode() {
...
dram_req_t *queue_scheduler::schedule(unsigned bank, unsigned curr_row) {
...
dram_req_t *queue_scheduler::schedule_pim() {
...
void queue_scheduler::update_switch_conditions()
{
  if (m_dram->m_dram_cycle >= m_next_update_cycle) {
    ...
    if (objective > DRAM_SCHED_QUEUE_OBJ_TOLERANCE) {
      if (queue_avg > queue_avg_pim) {
        m_mem_time_ratio = std::min(m_mem_time_ratio + \
            DRAM_SCHED_QUEUE_MEM_TIME_INCREMENTS, m_mem_time_ratio_hi);
        ...
        m_mem_time_ratio = std::max(m_mem_time_ratio - \
            DRAM_SCHED_QUEUE_MEM_TIME_INCREMENTS, m_mem_time_ratio_lo);

        if ((m_mem_time_ratio == m_mem_time_ratio_lo) && (m_min_pim_batches < \
              m_min_pim_batches_hi)) {
          m_mem_time_ratio = m_mem_time_ratio_hi;
          m_min_pim_batches += 1;
        }
    ...
    m_next_update_cycle = m_dram->m_dram_cycle + DRAM_SCHED_QUEUE_PHASE_LENGTH;
  }
}
